...
#include "wds.h" /* WDS_SUPPORT */

#include "linkmgt.h"
#include "mlme.h"
#include "mlmeApi.h"
#include "apMPrxy.h" /* MPRXY */
...
#endif
static int wlioctl_priv_wlparam(WL_NETDEV *dev, 
...
int LoadExternalFw(struct wlprivate *wlpptr, char *filename)
{
	mm_segment_t old_fs;
	int i = 0, j; 
...
struct iw_statistics *wlGetStats(WL_NETDEV *dev)
{
	...
	{
		wlFwGetHwStatsForWlStats(wlpptr, &((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats);
		linkInfo_p = mlmeApiGetStaLinkInfo(wlpptr);
		if (linkInfo_p)
		{
			((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats.qual.level = - linkInfo_p->wStats.qual.level;
			((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats.qual.noise = - linkInfo_p->wStats.qual.noise;
			((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats.qual.qual  = linkInfo_p->wStats.qual.qual;
			((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats.qual.updated = IW_QUAL_ALL_UPDATED;
			((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats.status = 0;
		}
	}

	WLDBG_EXIT(DBG_LEVEL_1);
	return &((struct wlprivate_data *)(wlpptr->wlpd_p))->wStats;
}

static int wlconfig_commit(WL_NETDEV *dev,struct iw_request_info *info,char *cwrq, char *extra)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p; /* WFA_TKIP_NEGATIVE */
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11; /* WFA_TKIP_NEGATIVE */
	...
	{
		/* If not root device (if root device wvmacSta_p->rootvmac is always NULL). */
		if(vmacSta_p->rootvmac)
		{           
			mib_Update();
		}
		else
...
static int wlget_name(WL_NETDEV *dev, struct iw_request_info *info, char *cwrq, char *extra)
{
	WLDBG_ENTER_INFO(DBG_LEVEL_1, "");
...
static int wlset_freq(WL_NETDEV *dev, struct iw_request_info *info, struct iw_freq *fwrq, char *extra)
{
	...
	else
	{
		...
		if(channel)
		{
			...
			*/
			DFS_STATUS DfsPresentInNOL(WL_PRIV *wlpptr, UINT8 channel );
			if( DfsPresentInNOL(wlpptr, channel ) )
			{
				rc = -EOPNOTSUPP;
				return rc ;
			}
...
static int wlget_freq(WL_NETDEV *dev, struct iw_request_info *info, struct iw_freq *fwrq, char *extra)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
...
static int wlget_sens(WL_NETDEV *dev,
struct iw_request_info *info,
...
static int wlget_range(WL_NETDEV *dev, struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
	struct wlprivate *wlpptr     = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t     *vmacSta_p  = wlpptr->vmacSta_p;
	iw_linkInfo_t    *linkInfo_p = NULL;
...
static int wlget_stats(WL_NETDEV *dev, struct iw_request_info *info, struct iw_statistics *stats, char *extra)
{
	struct wlprivate *wlpptr     = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t     *vmacSta_p  = wlpptr->vmacSta_p;
	iw_linkInfo_t    *linkInfo_p = NULL;
	...
	if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)
	{
		...
		wlFwGetHwStatsForWlStats(wlpptr, stats);
	}
...
static int wlset_scan(WL_NETDEV *dev,
struct iw_request_info *info,
struct iw_param *srq,
	char *extra)
{
...
static int wlget_scan(WL_NETDEV *netdev,
struct iw_request_info *info,
struct iw_point *srq,
	char *extra)
{
...
static int wlset_essid(WL_NETDEV *dev, struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
	struct wlprivate	*wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
...
static int wlget_essid(WL_NETDEV *dev,
struct iw_request_info *info,
...
static int wlset_bssid(WL_NETDEV *dev, struct iw_request_info *info, struct sockaddr *ap_addr, char *extra)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
...
static int wlset_rts(WL_NETDEV *dev,
struct iw_request_info *info,
...
static int wlget_rts(WL_NETDEV *dev,
struct iw_request_info *info,
struct iw_param *vwrq, char *extra)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
...
static int wlget_frag(WL_NETDEV *dev,
struct iw_request_info *info,
struct iw_param *vwrq, char *extra)
{
	WLDBG_ENTER_INFO(DBG_LEVEL_1, "");
...
static int wlget_wap(WL_NETDEV *dev,
struct iw_request_info *info,
...
static int wlset_encode(WL_NETDEV *dev,
struct iw_request_info *info,
struct iw_point *dwrq, char *extra)
{
	struct wlprivate	*wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
...
static int wlget_encode(WL_NETDEV *dev,
struct iw_request_info *info,
struct iw_point *dwrq, char *extra)
{
	struct wlprivate	*wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	...
	void *wrqu, char *extra)
{
	...
	if(is_the_param_applicable(param) && vmacSta_p->rootvmac){
		printk("This parameter cannot be set to virtual interface %s, please use %s instead!\n", dev->name, wlpptr->rootpriv->vmacSta_p->dev->name); 
		rc = -EOPNOTSUPP; 
		return rc; 
	} 
	...
	case WL_PARAM_REGIONCODE:
		...
		mib_SpectrumMagament_p->countryCode = value ; /* IEEE80211_DH */
	...
	case WL_PARAM_WPAWPA2MODE:
		{
			vmacEntry_t  *vmacEntry_p = NULL; /* WPS_CLIENT */
			STA_SYSTEM_MIBS *pStaSystemMibs ; /* WPS_CLIENT */
			...
			*(mib->mib_wpaWpa2Mode) = (UCHAR)value;
			...
			if ((value == 0) || ((value & 0x0000000F) == 0)) /* WPS_SUPPORT */
			{
				mib->Privacy->RSNEnabled = 0;
	...
	case WL_PARAM_SETKEYS:
		{
			...
			{
				if (extStaDb_SetRSNPwkAndDataTraffic(wlpptr,
				...
				if (extStaDb_SetPairwiseTSC(wlpptr,
					vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA ?
					(IEEEtypes_MacAddr_t *)GetParentStaBSSID((vmacEntry_p)->phyHwMacIndx) :
				(IEEEtypes_MacAddr_t *)wk.ik_macaddr, 
				...
				if( vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA )
				{
						...
						&& !pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2Enabled )
					{
						...
						{
							// TKIP
							wlFwSetWpaTkipMode_STA(wlpptr,  (UINT8 *)&pStaInfo->Addr);
						}
						else if ((pStaSecurityMibs->mib_RSNConfigUnicastCiphers_p->UnicastCipher[3] == 4))
						{
							// AES
							wlFwSetWpaAesMode_STA(wlpptr,  (UINT8 *)&pStaInfo->Addr);
						}
					}
					else
					{
						...
						else
						{
							...
							wlFwSetMixedWpaWpa2Mode_STA(wlpptr,  (UINT8 *)&pStaInfo->Addr);
						}
					}

					wlFwSetWpaWpa2PWK_STA(wlpptr, pStaInfo);
					if( pKeyMgmtInfoSta )
				...
				else
				{
					...
					{
						wlFwSetWpaWpa2PWK(wlpptr, pStaInfo);
						break;
					}
				}
			}
			else if ((0 < wk.ik_keyix) &&  (wk.ik_keyix< 4))
			{
				if (wk.ik_type == WL_CIPHER_TKIP)
				{
					if( vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP )
					{
						...
						wlFwSetWpaTkipGroupK(wlpptr, mib1->mib_MrvlRSN_GrpKey->g_KeyIndex);
					...
					else if( vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA )
					{
						...
						wlFwSetWpaTkipGroupK_STA(wlpptr,
				...
				else if (wk.ik_type == WL_CIPHER_CCMP)
				{
					if( vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP )
					{
						memcpy(mib1->mib_MrvlRSN_GrpKey->EncryptKey, &wk.ik_keydata[0], 16);
						mib1->mib_MrvlRSN_GrpKey->g_KeyIndex = (UINT8)wk.ik_keyix;
						wlFwSetWpaAesGroupK(wlpptr, mib1->mib_MrvlRSN_GrpKey->g_KeyIndex);
						//need to update shadow mib, when directly modify run-time mib.
						memcpy(mib->mib_MrvlRSN_GrpKey, 
							mib1->mib_MrvlRSN_GrpKey, sizeof(MRVL_MIB_RSN_GRP_KEY));
					}
					else if( vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA )
					{
						memcpy( mib_MrvlRSN_GrpKeyUr1[vmacEntry_p->phyHwMacIndx].EncryptKey,
							&wk.ik_keydata[0], TK_SIZE );
						wlFwSetWpaAesGroupK_STA(wlpptr,
							GetParentStaBSSID(vmacEntry_p->phyHwMacIndx),
							&mib_MrvlRSN_GrpKeyUr1[vmacEntry_p->phyHwMacIndx].EncryptKey[0],
							TK_SIZE);
	...
	case WL_PARAM_DELKEYS:
		{
			...
			if (extStaDb_SetRSNDataTrafficEnabled(wlpptr,(IEEEtypes_MacAddr_t *)wk.idk_macaddr, FALSE) != STATE_SUCCESS)
			{
				break;
			}
		}
	...
	case WL_PARAM_MLME_REQ:
		{
			...
			{
			case WL_MLME_DEAUTH:
				macMgmtMlme_SendDeauthenticateMsg(wlpptr, &mlme.im_macaddr, 0,
					mlme.im_reason);
				break;
			default:
				rc = -EOPNOTSUPP;
	...
	case WL_PARAM_COUNTERMEASURES:
		{
			if (value)
			{
				...
				macMgmtMlme_SendDeauthenticateMsg(wlpptr,&bcastMacAddr, 0, 
					IEEEtypes_REASON_MIC_FAILURE);
				extStaDb_RemoveAllStns(wlpptr,IEEEtypes_REASON_MIC_FAILURE);
			}
			else
			{
	...
	case WL_PARAM_11H_DFS_MODE:
		{
			...
			{
				wlFwSetRadarDetection(wlpptr, value );
			}
			else
			{
				rc = -EOPNOTSUPP;
	...
	case WL_PARAM_11H_CSA_START :
		{
			...
			{
				if( wlpptr->vdev[i] && wlpptr->vdev[i]->vmacSta_p->dev->flags & IFF_RUNNING )
				{
					SendChannelSwitchCmd(wlpptr->vdev[i], &ChannelSwitchCmd);
				}
			}
	...
	case WL_PARAM_PSHT_MANAGEMENTACT:
		{
			extern BOOLEAN macMgmtMlme_SendMimoPsHtManagementAction(WL_PRIV *wlpptr,IEEEtypes_MacAddr_t *Addr, UINT8 mode);
			extern int wlFwSetMimoPsHt(WL_PRIV *wlpptr, UINT8 *addr, UINT8 enable, UINT8 mode);
			...
			case 3:
				*(mib->mib_psHtManagementAct) = (UINT8 )value;
				if (macMgmtMlme_SendMimoPsHtManagementAction(wlpptr, (IEEEtypes_MacAddr_t *)&addr, *(mib->mib_psHtManagementAct)) == TRUE)
				{
					entries = extStaDb_entries(wlpptr, 0);
					staBuf = kmalloc(entries*sizeof(STA_INFO) ,GFP_KERNEL);
					if (staBuf != NULL)
					{
						extStaDb_list(wlpptr, staBuf, 1);
						...
						if(entries)
						{
							...
							for(i=0; i<entries; i++)
							{
								if ( (pStaInfo = extStaDb_GetStaInfo(wlpptr, (IEEEtypes_MacAddr_t *)listBuf, 0)) != NULL )
								{
									if ((pStaInfo->State == ASSOCIATED) && (pStaInfo->ClientMode == NONLY_MODE))
									{
										...
										wlFwSetMimoPsHt(wlpptr, listBuf, enable, mode);
									}
	...
	case WL_PARAM_INTOLERANT:
		...
		break ; /* COEXIST_20_40_SUPPORT */
	
	case WL_PARAM_STAMODE:
		{
			vmacEntry_t  *vmacEntry_p     = NULL;
			WL_PRIV  *wlpptrSta     = NULL;
			...
			*(mib->mib_STAMode) = (UCHAR)value;
			vmacEntry_p = sme_GetParentVMacEntry(((vmacApInfo_t *) wlpptr->vmacSta_p)->VMacEntry.phyHwMacIndx);
			wlpptrSta = (WL_PRIV *)vmacEntry_p->privInfo_p;
	...
	case WL_PARAM_STASCAN:
		{
			...
			if((vmacEntry_p = sme_GetParentVMacEntry(((vmacApInfo_t *) wlpptr->vmacSta_p)->VMacEntry.phyHwMacIndx)) == NULL)
			{
				rc = -EFAULT;
				break;
			}
			...
			if (!mlmeAssociatedFlag && (wlpptr->vmacSta_p->InfUpFlag))
			{
				//printk("stopping BSS \n");
			...
			if (*(mib->mib_MCastPrxy))
			{
				...
				if (*(mib->mib_consectxfaillimit)== CONSECTXFAILLIMIT)
				{
					*(mib->mib_consectxfaillimit) = _CONSECTXFAILLIMIT;
					wlFwSetConsecTxFailLimit(wlpptr, _CONSECTXFAILLIMIT);				
				}
			...
			{	/*Set back to default value*/
				if (*(mib->mib_consectxfaillimit)== _CONSECTXFAILLIMIT)
				{
					*(mib->mib_consectxfaillimit) = CONSECTXFAILLIMIT;
					wlFwSetConsecTxFailLimit(wlpptr, CONSECTXFAILLIMIT);
				}
			}
		}
		break; /* MPRXY */
...
static int wlioctl_priv_get_wlparam(WL_NETDEV *dev,
struct iw_request_info *info,
	void *wrqu, char *extra)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	...
	case WL_PARAM_CARDDEVINFO:
		{
			*param  = ((struct wlprivate_data *)(wlpptr->wlpd_p))->CardDeviceInfo;
	...
	case WL_PARAM_11H_STA_MODE :
		...
		break;
	/* IEEE80211_DH */
	case WL_PARAM_WDSMODE: /* WDS_SUPPORT */
	...
	case WL_PARAM_11hDFSMODE:
		if( wlpptr->wlpd_p->pdfsApMain )
			*param = DfsGetCurrentState(wlpptr->wlpd_p->pdfsApMain);
		else
			*param = 0 ;
		break ;
	/* DFS_SUPPORT */
...
static int wlioctl_priv_bss_start(WL_NETDEV *dev,
struct iw_request_info *info,
	void *wrqu, char *extra)
{
	int rc = 0;
...
int wlSetupWEHdlr(WL_NETDEV *netdev)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
...
int wlIoctlSet(WL_NETDEV *netdev, int cmd, char *param_str, int param_len, char *ret_str, UINT16 *ret_len)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	...
	WLDBG_ENTER(DBG_LEVEL_1);
	if(is_the_cmd_applicable(cmd) && wlpptr->vmacSta_p->rootvmac){
		rc = -EOPNOTSUPP; 
		WL_FREE(param);
		return rc; 
	} 
	...
	case WL_IOCTL_SET_CLIENT:
		{
			...
			/* Get VMAC structure of the rootpriv*/
			if(!wlpptr->rootpriv)
			{
			...
			wlFwRemoveMacAddr(wlpptr, &vmacEntry_p->vmacAddr[0]);	
			if (mlmeAssociatedFlag)
				 	cleanupAmpduTx(wlpptr, (UINT8 *)&mlmeBssid[0]);
			...
			else
			{
				...
				memcpy(macaddr, wlpptr->rootpriv->vmacSta_p->dev->dev_addr, 6);
				for (index = 0; index< NUMOFAPS; index++)
				{
					...
					if(i)
					{
						macaddr[0]=wlpptr->rootpriv->vmacSta_p->dev->dev_addr[0] |((i<<2)|0x2);
					}
					bssidmask |=1<<i;
				}
	...
	case WL_IOCTL_SET_WDS_PORT:
		{
			...
			if (!setWdsPort(wlpptr, MacAddr, index, wdsPortMode))
			{
				rc = -ENODEV;
			}
			sprintf(wlpptr->vmacSta_p->wdsPort[index].netDevWds->name, DRV_NAME_WDS,wlpptr->netDev->name, (int )index);
			memcpy(wlpptr->vmacSta_p->wdsPort[index].netDevWds->dev_addr, wlpptr->vmacSta_p->macStaAddr, 6);
		}
		break;
	/* WDS_SUPPORT */
	...
	case WL_IOCTL_SETCMD:
		{
			...
			else if ((strcmp(param[0], "extfw") == 0) || (strcmp(param[0], "mfgfw") == 0))
			{
				...
				if(!LoadExternalFw(wlpptr, param[1]))
				{
					/* No file is loaded */
					rc = -EFAULT;
					break;
			...
			{
				extern int wlFwMfgCmdIssue(WL_PRIV *, char *pData,  char *pDataOut);

				char *pOut = cmdGetBuf;
				char *pIn = param_str + strlen(param[0]) + 1;
				int len = *(UINT16 *)(pIn+2);

				wlFwMfgCmdIssue(wlpptr, pIn, (pOut+4));
			...
			else if	(strcmp(param[0], "fwrev") == 0)
			{
				...
				if( strcmp(param[0], "addba") == 0)
				{
					...
					{
						wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[0]=macaddr[0];
						wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[1]=macaddr[1];
						wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[2]=macaddr[2];
						wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[3]=macaddr[3];
						wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[4]=macaddr[4];
						wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[5]=macaddr[5];
						...
						{
							wlpptr->wlpd_p->Ampdu_tx[stream].timer = TimerInit();
							wlpptr->wlpd_p->Ampdu_tx[stream].initTimer= 1;
						}
						TimerDisarm(wlpptr->wlpd_p->Ampdu_tx[stream].timer);
						wlpptr->wlpd_p->Ampdu_tx[stream].wlpptr = wlpptr;
						TimerFireIn(wlpptr->wlpd_p->Ampdu_tx[stream].timer, 1, &AddbaTimerProcess, (UINT8 *)&wlpptr->wlpd_p->Ampdu_tx[stream], 1000);
					}
					else
					{
					...
					if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)
						SendAddBAReqSta(wlpptr, macaddr, tid,1, seqNo,wlpptr->wlpd_p->Ampdu_tx[stream].DialogToken);
					else
						SendAddBAReq(wlpptr,macaddr, tid,1, seqNo,wlpptr->wlpd_p->Ampdu_tx[stream].DialogToken);  /** Only support immediate ba **/
				...
				else if(strcmp(param[0],"ampdu_stat") == 0)
				{
					...
					for(i=0; i < MAX_SUPPORT_AMPDU_TX_STREAM; i++)
					{
						...
						for(j=0;j<6;j++)
						{
							printk("%x ",wlpptr->wlpd_p->Ampdu_tx[i].MacAddr[j]);
						}
						printk("tid %x Inuse %x timeout %d pps %d\n",wlpptr->wlpd_p->Ampdu_tx[i].AccessCat,wlpptr->wlpd_p->Ampdu_tx[i].InUse,
							(int)wlpptr->wlpd_p->Ampdu_tx[i].TimeOut,(int)wlpptr->wlpd_p->Ampdu_tx[i].txa_avgpps );
						printk("\n");
					}
				...
				else if( strcmp(param[0], "delba") == 0)
				{
					...
					for(i=0;i<MAX_SUPPORT_AMPDU_TX_STREAM;i++)
					{
						ampdu_tx_p = &wlpptr->wlpd_p->Ampdu_tx[i];
						printk(" AMacaddr2 %x %x %x %x %x %x\n",ampdu_tx_p->MacAddr[0],ampdu_tx_p->MacAddr[1],ampdu_tx_p->MacAddr[2],ampdu_tx_p->MacAddr[3],ampdu_tx_p->MacAddr[4],ampdu_tx_p->MacAddr[5]);
						printk(" Macaddr2 %x %x %x %x %x %x\n",macaddr2[0],macaddr2[1],macaddr2[2],macaddr2[3],macaddr2[4],macaddr2[5]);
						printk(" tid = %x , In Use = %x \n*******\n",ampdu_tx_p->AccessCat,ampdu_tx_p->InUse);
						disableAmpduTx(wlpptr,macaddr2, tid);
					}
				}
				else if( strcmp(param[0], "del2ba") == 0)  /** fake command use by WIFI testbed **/
				{
					...
					SendDelBA2(wlpptr, macaddr2, tid);
				}
				...
				else if(strcmp(param[0], "intolerant40") == 0)
				{
					...
					else
					{
						...
						{
							extern int wlFwSet11N_20_40_Switch(WL_PRIV * wlpptr, UINT8 mode);

							*(vmacSta_p->ShadowMib802dot11->mib_FortyMIntolerant) = mode;
							*(mib->USER_ChnlWidth )= mode;

							wlFwSet11N_20_40_Switch(wlpptr, mode);
				...
				{
					extern int wlFwSet11N_BF_Mode(WL_PRIV *, UINT8 bf_option, UINT8 bf_csi_steering, UINT8 bf_mcsfeedback, UINT8 bf_mode,
					...
					wlFwSet11N_BF_Mode(wlpptr, option,csi_steering,mcsfeedback,mode,interval,slp,power);	  
				...
				{
					extern int wlFwSetNoAck(WL_PRIV *, UINT8 Enable);
					UINT8 Enable;
					Enable = atohex2(param[1]);
					printk("Set NoACK= %x\n", Enable);
					wlFwSetNoAck(wlpptr, Enable);
				}

				else if(strcmp(param[0], "NoSteer") == 0)
				{
					extern int wlFwSetNoSteer(WL_PRIV *, UINT8 Enable);
					UINT8 Enable;
					Enable = atohex2(param[1]);
					printk("Set NoSteer = %x\n", Enable);
					wlFwSetNoSteer(wlpptr, Enable);
				}
				else if(strcmp(param[0], "SetCDD") == 0)
				{
					extern int wlFwSetCDD(WL_PRIV *, UINT32 cdd_mode);
					UINT32 cdd_mode;
					cdd_mode = atohex2(param[1]);
					printk("Set CDD= %x\n", (unsigned int)cdd_mode);
					wlFwSetCDD(wlpptr, cdd_mode);
				}
				else if(strcmp(param[0], "SetTxHOP") == 0)
				{
					extern int wlFwSetTxHOP(WL_PRIV *, UINT8 Enable, UINT8  TxHopStatus);
					UINT8 Enable, TxHopStatus;
					Enable = atohex2(param[1]);
					TxHopStatus = atohex2(param[2]);
					printk("Set Txhop= %x status=%x \n", Enable, TxHopStatus);
					wlFwSetTxHOP(wlpptr, Enable,  TxHopStatus);
				}
...
#define LINECHAR	16
						...
						if (wlFwGetBeacon(wlpptr, pBcn, &len) == FAIL)
						{
							rc = -EFAULT;
							WL_FREE(pBcn);
							break;
					...
					else if	((strcmp(param[0], "annex") == 0) || (strcmp(param[0], "readeepromhdr") == 0))
					{
						...
						if (wlFwGetCalTable(wlpptr, (UINT8)annex, (UINT8)index) == FAIL)
						{
							rc = -EFAULT;
							break;
						}
					...
					{
						wlFwGetAddrtable(wlpptr);
					}
					else if ((strcmp(param[0], "getfwencrinfo") == 0))
					{
						char macaddr[6];
						getMacFromString(macaddr, param[1]);
						wlFwGetEncrInfo(wlpptr, macaddr);
					}
					else if	((strcmp(param[0], "setreg") == 0) )
					{
						...
						if (strcmp(param[1], "mac") == 0)
						{
							if(set == WL_SET)
							{
								PciWriteMacReg(wlpptr, reg, val);
							} else
								val = PciReadMacReg(wlpptr, reg);
						...
						{
							if(set == WL_SET)
								*(volatile unsigned int *)((unsigned int)wlpptr->ioBase0 + reg) = val;
							else
								val = *(volatile unsigned int *)((unsigned int)wlpptr->ioBase0 + reg);
						...
						{
							if(set == WL_SET)
								*(volatile unsigned int *)((unsigned int)wlpptr->ioBase1 + reg) = val;
							else
								val = *(volatile unsigned int *)((unsigned int)wlpptr->ioBase1 + reg);
						...
						else if (strcmp(param[1], "addr") == 0)
						{
							...
							if(set == WL_SET)
							{
								wlFwGetAddrValue(wlpptr, reg, 4, addr_val, 1);
							}
							else
								wlFwGetAddrValue(wlpptr, reg, 4, addr_val, 0);
					...
					{
						DebugCmdParse(wlpptr, param_str+6);
						break;
					}
					else if	((strcmp(param[0], "memdump") == 0) )
					{
						...
						{ 
							...
							for(k = 0; k < length; k+=256)
							{
								for(i = 0; i < 256; i+=4)
								{
									...
									val = *(volatile unsigned int *)((unsigned int)wlpptr->ioBase1 + offset + i+k);

									//val = PciReadMacReg(wlpptr, offset+i);
									if(i %16 ==0)
									{
						...
						{ 
							...
							for(k = 0; k < length; k+=256)
							{
								for(i = 0; i < 256; i+=4)
								{
									...
									val = *(volatile unsigned int *)((unsigned int)wlpptr->ioBase0 + offset + i+k);
						...
						else if (strcmp(param[1], "rf") == 0)
						{
							...
							for(i = 0; i < length; i++)
							{
								wlRegRF(wlpptr, WL_GET,  offset+i, &val);
								if(i %8 ==0)
								{
									sprintf(buf+j, "\n%02x: ",(int)(offset+i));
									j = strlen(buf);
						...
						else if (strcmp(param[1], "bb") == 0)
						{
							...
							for(i = 0; i < length; i++)
							{
								wlRegBB(wlpptr, WL_GET,  offset+i, &val);
								if(i %8 ==0)
								{
									sprintf(buf+j, "\n%02x: ",(int)(offset+i));
									j = strlen(buf);
						...
						{	
							...
							if( length > 64 )
								length = 64 ;
							if(wlFwGetAddrValue(wlpptr, addr, length, addr_val,0 ) )
							{
					...
					else if( strcmp(param[0], "getratetable") == 0)
					{
						...
						{						
							...
							if(pRateTable)
							{
								...
								getMacFromString(macaddr, param[1]);
								wlFwGetRateTable(wlpptr, (UINT8 *)macaddr,(UINT8 *)pRateTable, size);
					...
					else if	((strcmp(param[0], "rptrmode") == 0) )
					{
						...
						{
							extern UINT16 ethStaDb_list(WL_PRIV *);
							ethStaDb_list(wlpptr);
						}
						else if	((strcmp(param[1], "addmac") == 0) )	
						{
							getMacFromString(macaddr, param[2]);
							if ((pStaInfo = extStaDb_GetStaInfo(wlpptr,(IEEEtypes_MacAddr_t *)macaddr, 0)) != NULL )
							{
						...
						else if	((strcmp(param[1], "delmac") == 0) )	
						{
							...
							{
								pStaInfo->StaType = 0;
								ethStaDb_RemoveStaPerWlan(wlpptr, (IEEEtypes_MacAddr_t *)macaddr);
							}
						} else {
							rc = -EFAULT;
							break;
					...
					{
						extern int wlFwSetRCcal(WL_PRIV *);
						wlFwSetRCcal(wlpptr);
                        printk("RC Cal done\n");
					}
					else if(strcmp(param[0], "gettemp") == 0)
					{
						extern int wlFwGetTemp(WL_PRIV *);
						wlFwGetTemp(wlpptr);
					}
					...
					else if	((strcmp(param[0], "txfaillimit") == 0))
					{
						...
						if(!wlFwSetConsecTxFailLimit(wlpptr, *(mib->mib_consectxfaillimit)))
						{	
					...
					else if ((strcmp(param[0], "gettxfaillimit") == 0))
					{
						...
						if(!wlFwGetConsecTxFailLimit(wlpptr, (UINT32*)&val))
						{
							if(val)
	...
	case WL_IOCTL_SET_APPIE:
		{
			...
			else
			{
				...
				memset(&vmacSta_p->thisbeaconIE, 0, sizeof(WSC_BeaconIE_t));
				ieType = 0;
				if(wlFwSetWscIE(wlpptr, ieType, &APWSCIE ))
				{
...
int wlIoctlGet(WL_NETDEV *netdev, int cmd, char *param_str,
			   int param_len, char *ret_str, UINT16 *ret_len)
{
	...
	case WL_IOCTL_GET_VERSION:
		{
			UINT8 *pVer = (UINT8 *)&wlpptr->hwData.fwReleaseNumber;
	...
	case WL_IOCTL_GET_STALISTEXT:
		{
			...
			extStaDb_list(wlpptr,sta_buf, 1);
			...
			if(entries)
			{
				...
				for(i=0; i<entries; i++)
				{
					if ( (pStaInfo = extStaDb_GetStaInfo(wlpptr,(IEEEtypes_MacAddr_t *)show_buf, 0)) == NULL )
					{
						WL_FREE(sta_buf);
						rc = -EFAULT;
						return rc;
	...
	case WL_IOCTL_GET_TXPOWER:
		{
			...
			wlFwGettxpower(wlpptr, powlist, mib->PhyDSSSTable->CurrChan,
	...
	case WL_IOCTL_GET_IE:
		{
			...
			{
				if (extStaDb_GetRSN_IE(wlpptr,&IEReq.macAddr, (UINT8 *)&IEReq.IE) != STATE_SUCCESS)
				{
					rc = -EFAULT;
					break;
				}
	...
	case WL_IOCTL_GET_WDS_PORT:
		{
			...
			{
				getWdsModeStr(wdsModeStr, wlpptr->vmacSta_p->wdsPort[index].wdsPortMode);

				sprintf(out_buf, "ap0wds%x HWaddr %x:%x:%x:%x:%x:%x  802.%s Port %s \n",
					index,
					wlpptr->vmacSta_p->wdsPort[index].wdsMacAddr[0],
					wlpptr->vmacSta_p->wdsPort[index].wdsMacAddr[1],
					wlpptr->vmacSta_p->wdsPort[index].wdsMacAddr[2],
					wlpptr->vmacSta_p->wdsPort[index].wdsMacAddr[3],
					wlpptr->vmacSta_p->wdsPort[index].wdsMacAddr[4],
					wlpptr->vmacSta_p->wdsPort[index].wdsMacAddr[5],
					wdsModeStr, wlpptr->vmacSta_p->wdsPort[index].active?"Active":"Inactive");
				out_buf += strlen(out_buf); 
			}
		}
		break; /* WDS_SUPPORT */
	...
	case WL_IOCTL_GETCMD :
		{
			...
			*param[1] = '\0';
			...
			if((strcmp(param[0], "get11hNOCList") == 0))
			{
				if( wlpptr->wlpd_p->pdfsApMain )
				{
					DfsPrintNOLChannelDetails( wlpptr->wlpd_p->pdfsApMain, buf, 200 );
				}
				else
				{
					rc = -EFAULT;
	...
	case WL_IOCTL_GET_STASCAN:
		{
			...
			for(i=0; i < tmpNumScanDesc[vmacSta_p->VMacEntry.phyHwMacIndx]; i++)
			{
				...
				sprintf(out_buf, "#%d SSID=%s <%02x:%02x:%02x:%02x:%02x:%02x> %d -%d %s %s %s\n",
					i+1,
...
case 4:				/* a.b.c.d -- 8.8.8.8 bits */
	...
	***************************************************************************************/
	...
	struct net_device *get_wifi_qos_cap(char* ifName, wifi_qos_cap_t* cap)
	{
		...
		{
			netdev = ((WL_PRIV *)global_private_data[i].appriv.rootwlpptr)->vmacSta_p->dev;
	...
	***************************************************************************************/
	int get_wifi_wmm_queue_status(struct net_device *pdev, wmm_q_status_t *status)
	{
		...
		{
			memcpy((char *)status, (char *)&(((struct wlprivate_data *)(wlpptr->wlpd_p))->fwDescCnt[0]), sizeof(wmm_q_status_t));
			return 0;
		}
		return -1;
	}
