	...
	while(num--)
	{
		...
		while (((struct wlprivate_data *)(wlpptr->wlpd_p))->fwDescCnt[num] < MAX_NUM_TX_DESC && (skb= QUEUE_DEQUEUE(&((struct wlprivate_data *)(wlpptr->wlpd_p))->txQ[num])) != 0)
		{
			if(skb_shinfo(skb)->tso_size){ 
				((struct wlprivate_data *)(wlpptr->wlpd_p))->privStats.tsoframecount++;
...
int wlxmit(struct net_device *netdev, struct sk_buff *skb, struct wlxmit_param *param)
{
	...
	UINT16 amsdulen = param->totallen;
	UINT16 flag = param->flags >> 16;
	...
	if(!(flag & PFU_DESCRIPTOR_FLAG_SW_AMSDU))
	{
...
#define SRC_ADD_OFFSET 16
...

    {
		...
		if(flag & PFU_DESCRIPTOR_FLAG_SW_FISRT_AMSDU)
			*pFwLen = ENDIAN_SWAP16(amsdulen - NBR_BYTES_COMPLETE_TXFWHEADER + wep_padding+data_padding);
		else
