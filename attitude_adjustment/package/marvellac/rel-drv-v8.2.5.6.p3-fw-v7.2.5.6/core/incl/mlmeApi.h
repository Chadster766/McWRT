...
typedef struct tx80211_MgmtMsg_t
{
    ...
    // A value of 0xFF in this member indicates "do not use this value". 0xFF cannot be valid since 
...
struct wps_scan_result {
	...
	size_t rsn_ie_len;
	UINT8 wmm_ie[SSID_MAX_WPA_IE_LEN];
	size_t wmm_ie_len;
	UINT8 wps_ie[SSID_MAX_WPS_IE_LEN];      /* WPS IE */
	size_t wps_ie_len;
	int freq;
	UINT16 caps;
	int qual;
...
extern dot11MgtFrame_t *mlmeApiAllocMgtMsg(WL_PRIV *wlpptr,UINT8 phymac);
...
extern void mlmeApiSetMacAddrByMacId(vmacStaInfo_t * vStaInfo_p);
extern iw_linkInfo_t *mlmeApiGetStaLinkInfo(WL_PRIV *);
...
extern void SendDelBASta(WL_PRIV *wlpptr,IEEEtypes_MacAddr_t StaAddr, UINT8 tsid);
void InitClientPeerInfo(WL_PRIV *);
...
extern void SendAddBAReqSta(WL_PRIV *wlpptr,IEEEtypes_MacAddr_t StaAddrA, UINT8 tsid,
