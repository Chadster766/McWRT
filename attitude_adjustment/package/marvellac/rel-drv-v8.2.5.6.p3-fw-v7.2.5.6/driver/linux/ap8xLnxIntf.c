...
static int wlFwSetHwSpecs(WL_NETDEV *netdev)
{
	...
	pCmd->WcbBase[0]	 = ENDIAN_SWAP32(((struct wlprivate_data *)(wlpptr->wlpd_p))->descData[0].pPhysTxRing);
#if NUM_OF_DESCRIPTOR_DATA >3
	for (i = 1; i < TOTAL_TX_QUEUES; i++)
		pCmd->WcbBase[i]	 = ENDIAN_SWAP32(((struct wlprivate_data *)(wlpptr->wlpd_p))->descData[i].pPhysTxRing);
#endif
