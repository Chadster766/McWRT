...
static inline int tcp_ack_sequence_no(struct wlprivate *wlpptr, struct sk_buff *skb , struct net_device *netdev, u_int32_t *seq)
{
	...
	if((iph->protocol == IPPROTO_TCP) && (htons(ETH_P_IP) == skb->protocol)){
		if((th->ack == 1) && (htons(iph->tot_len) == (iph->ihl * 4 + th->doff * 4)))
		{
			...
			if(th->syn || th->fin)
			{
				//do not mark syn and fin as drop candidate
				skb->cb[1] = 0;
				return 0;
			}
			if(debug_tcpack)
...
int wlDataTx(struct sk_buff *skb, struct net_device *netdev)
{
	...
	{	
		...
		return 0; // return since we don't need new incoming pkt flush out pkts stuck in txq. timer_routine will flush out stuck pkts in txq
	}
	else
	{
	/* MPRXY */
...
schedule: 
	...
	* (wlDataTxHdl may not be able to send all pkts in one interrupt)
	*/
